...
if (documents.length != 0) {
    ...
    } catch (e) { alert('An error has occurred while collecting the file name.'); }
...
function saveFiles(makeThumbs) {
    ...
    try {
        ...
        if (widescreen && lt) { // Only save L3 TH if widescreen is true
...
function mergeLowerThird () {
    ...
    if (mergeFolder != undefined) {
        if (hasLayerMask('LOWER THIRD')) { 
            ...
            var desc = new ActionDescriptor();  
            var ref = new ActionReference();  
            ref.putClass( charIDToTypeID('layerSection') );  
            desc.putReference( charIDToTypeID('null'), ref );  
            var ref = new ActionReference();  
            ref.putEnumerated( charIDToTypeID('Lyr '), charIDToTypeID('Ordn'), charIDToTypeID('Trgt') );  
